...
public InsertCharacter()
  ...
  public synchronized void updateCharacter(Character characterToUpdate) throws SQLException
  {  Statement st = c.createStatement();
    String query = "DELETE FROM \"Characters\".\"Characters\" WHERE id="+ characterToUpdate.getId()+";";
    st.executeUpdate(query);

    insertCharacterWithID(characterToUpdate);
  }
  public void insertCharacterWithID(Character cha) throws SQLException
  {
    /**
     * Character insertion method. Gets character data from the argument character, and parses it into SQL format.
     * @param cha Character to be inserted into the database.
    ...
    Statement st = c.createStatement();
    String query =
        "INSERT INTO \"Characters\".\"Characters\" (\"name\", \"id\" \"level\", \"XP\", \"hitPointRolled\", \"alignment\", \"faith\", \"physicalTraits\", \"money\", \"abilities\", \"backgroundName\", \"classes\", \"classLevels\", \"extraFeatNames\", \"username\", \"groupName\", \"equipment_names\", \"equipment_amount\", \"abilitiesRolled\") VALUES ("
            + cha.getName() + ", " +cha.getId()+", " + cha.getLevel() + ", " + cha.getXp() + ", "
            + cha.getRolledHp() + ", " + cha.getAlignment() + ", " + cha
            .getFaith() + ", " + cha.getPhysicalCharacteristics() + ", ";
    //add money
    int[] money = cha.getMoney();
